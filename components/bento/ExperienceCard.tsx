...
export function ExperienceCard({ experience, className }: ExperienceCardProps) {
  const { t } = useLanguage();
  const expT = t.experiences[experience.id as keyof typeof t.experiences];
      ...
      <SpotlightCard className="h-full flex flex-col justify-between p-6 cursor-pointer">
        <div>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-medium text-muted uppercase tracking-wider">{experience.company}</p>
          ...
          <p className="mt-3 text-sm text-muted line-clamp-2 leading-relaxed">{expT.shortDescription}</p>
        </div>
