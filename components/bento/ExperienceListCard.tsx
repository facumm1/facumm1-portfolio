...
export function ExperienceListCard() {
  ...
  return (
    <SpotlightCard className="col-span-12 md:col-span-7 flex flex-col p-6">
      <span className="text-xs font-medium text-muted uppercase tracking-wider">{t.ui.experience}</span>

      <div className="mt-4 flex flex-col">
        {experiences.map((exp, i) => {
          const expT = t.experiences[exp.id as keyof typeof t.experiences];
          return (
            <ExperienceExpandableCard key={exp.id} experience={exp} translations={expT}>
              <div
                className={cn(
